...
static int lib_polyobj_moveXY(lua_State *L)
{
	...
	fixed_t x = luaL_checkfixed(L, 2);
	fixed_t y = luaL_checkfixed(L, 3);
	boolean checkmobjs = lua_opttrueboolean(L, 4); // Just silently discard checkmobjs
    (void)checkmobjs;
	NOHUD
	INLEVEL
	if (!po)
		return LUA_ErrInvalid(L, "polyobj_t");
	lua_pushboolean(L, Polyobj_moveXY(po, x, y));
	return 1;
}
...
static int lib_polyobj_rotate(lua_State *L)
{
	...
	if (!po)
		return LUA_ErrInvalid(L, "polyobj_t");
	lua_pushboolean(L, Polyobj_rotate(po, delta, turnthings));
	return 1;
}
