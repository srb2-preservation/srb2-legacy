...
static void R_Subsector(size_t num)
{
	...
	floorcenterz = frontsector->f_slope ? P_GetZAt(frontsector->f_slope, frontsector->soundorg.x, frontsector->soundorg.y) : frontsector->floorheight;

	ceilingcenterz = frontsector->c_slope ? P_GetZAt(frontsector->c_slope, frontsector->soundorg.x, frontsector->soundorg.y) : frontsector->ceilingheight;
		...
		&& sectors[frontsector->heightsec].ceilingpic == skyflatnum)))
	{
		floorplane = R_FindPlane(frontsector->floorheight, frontsector->floorpic, floorlightlevel,
			frontsector->floor_xoffs, frontsector->floor_yoffs, frontsector->floorpic_angle, floorcolormap, NULL , NULL , frontsector->f_slope, frontsector->floor_scale);
	}
	else
		floorplane = NULL;
		...
		&& sectors[frontsector->heightsec].floorpic == skyflatnum)))
	{
		ceilingplane = R_FindPlane(frontsector->ceilingheight, frontsector->ceilingpic,
			...
			ceilingcolormap, NULL, NULL , frontsector->c_slope, frontsector->ceiling_scale);
	}
	else
		ceilingplane = NULL;
	...
	if (frontsector->ffloors)
	{
		...
		for (rover = frontsector->ffloors; rover && numffloors < MAXFFLOORS; rover = rover->next)
		{
				...
				|| (viewz > heightcheck && (rover->flags & FF_BOTHPLANES))))
			{
				...
				ffloor[numffloors].plane = R_FindPlane(*rover->bottomheight, *rover->bottompic,
					...
					*rover->bottomyoffs, *rover->bottomangle, frontsector->lightlist[light].extra_colormap, rover
					, NULL , *rover->b_slope, frontsector->floor_scale);
			...
			heightcheck = *rover->t_slope ? P_GetZAt(*rover->t_slope, viewx, viewy) : *rover->topheight;

			planecenterz = *rover->t_slope ? P_GetZAt(*rover->t_slope, frontsector->soundorg.x, frontsector->soundorg.y) : *rover->topheight;
				...
				|| (viewz < heightcheck && (rover->flags & FF_BOTHPLANES))))
			{
				...
				ffloor[numffloors].plane = R_FindPlane(*rover->topheight, *rover->toppic,
					*frontsector->lightlist[light].lightlevel, *rover->topxoffs, *rover->topyoffs, *rover->topangle,
					...
					, NULL , *rover->t_slope, frontsector->ceiling_scale);
		...
		while (po)
		{
				...
				&& (viewz < polysec->floorheight))
			{
				...
				ffloor[numffloors].plane = R_FindPlane(polysec->floorheight, polysec->floorpic,
						...
						NULL, po, NULL, polysec->floor_scale); // will ffloors be slopable eventually?;
...
//				ffloor[numffloors].ffloor = rover;
				...
				&& (viewz > polysec->ceilingheight))
			{
				...
				ffloor[numffloors].plane = R_FindPlane(polysec->ceilingheight, polysec->ceilingpic,
					...
					, po, NULL, polysec->ceiling_scale); // will ffloors be slopable eventually?
